...
impl Default for WorldSettings {
    fn default() -> Self {
        ...
        Self {
            ...
            tile_scale: scale, 
            render_distance: 50, // INCREASED: Larger visible area
            island_size: 60.0,   // INCREASED: Larger procedural island
            height_step: 2.0, 
        }
    }
}
...
enum TileType {
    // Terrain
    DeepWater, Water, Sand, Grass, Forest, Dirt, 
    Hill, Mountain, MountainSnow,
    
    // Biomes/Specifics
    SandDesert, // sand-desert.glb
    Stone,      // stone.glb
    ...
    // Buildings - Economy
    Market,     // building-market.glb
    Farm,       // building-farm.glb
    Mill,       // building-mill.glb
    Mine,       // building-mine.glb
    Smelter,    // building-smelter.glb
    Lumber,     // dirt-lumber.glb
    Port,       // building-port.glb
    Dock,       // building-dock.glb
    
    // Buildings - Military/Magic
    Castle,     // building-castle.glb
    Tower,      // building-tower.glb
    Archery,    // building-archery.glb
    Barracks,   // building-cabin.glb (reused)
    WizardTower,// building-wizard-tower.glb
    Wall,       // building-wall.glb
    
    // Units/Decor
    Sheep,      // building-sheep.glb
    Ship,       // unit-ship-large.glb
    Watermill,  // building-watermill.glb
    WallTower,
}

// Simple pseudo-random noise for determinism
fn pseudo_noise(q: i32, r: i32, seed: f32) -> f32 {
    let s = (q as f32 * 12.9898 + r as f32 * 78.233 + seed).sin() * 43758.5453;
    s - s.floor()
}

// Maps neighbor mask to Kenney's specific intersection filenames
fn get_advanced_connection(mask: u8, prefix: &str) -> (String, f32) {
    let m6 = mask & 0b111111;
    if m6 == 0 { return (format!("{}-start.glb", prefix), 0.0); }

    // (Pattern Mask, Name suffix)
    // We include all of Kenney's specific types here
    let patterns = [
        (0b000001, "end"),
        (0b001001, "straight"),
        (0b000101, "corner"),
        ...
        // The complex intersections from your list
        (0b000111, "intersectionA"), // 3-way adjacent
        (0b001011, "intersectionB"), // 2-adj + 1-gap
        (0b010011, "intersectionC"), // 2-adj + 2-gap
        (0b010101, "intersectionD"), // Y-shape (Balanced)
        (0b001111, "intersectionE"), // 4-way adjacent
        (0b010111, "intersectionF"), // 3-adj + 1-gap
        (0b011011, "intersectionG"), // X-shape (Balanced)
        (0b011111, "intersectionH"), // 5-way
    ];

    for r in 0..6 {
        // Rotate mask left to see if it fits a canonical pattern
        let mut shifted = m6;
        for _ in 0..r { shifted = ((shifted >> 1) | (shifted << 5)) & 0b111111; }
        
        for (pattern, suffix) in patterns {
            if shifted == pattern {
                let rot_radians = -(r as f32) * PI / 3.0;
                return (format!("{}-{}.glb", prefix, suffix), rot_radians);
            }
        }
    }

    // If 6 connections or no match
    (format!("{}-crossing.glb", prefix), 0.0)
}

fn generate_hex_data(q: i32, r: i32, seed: f32, island_size: f32) -> HexData {
    // 1. Calculate Distance from Center (for Island shape)
    let dist = (q.abs() + (q + r).abs() + r.abs()) as f32 / 2.0;
    
    // 2. Base Noise (Elevation)
    let n_elev = pseudo_noise(q, r, seed);
    let n_bio = pseudo_noise(q + 100, r - 100, seed); // Biome noise
    let n_detail = pseudo_noise(q * 2, r * 2, seed + 50.0); // Detail noise

    // 3. Shape the Island
    // Height decreases as we get further from center
    let height_val = (n_elev * 10.0) - (dist / island_size * 12.0);
    
    let height = if height_val < -2.0 { -2 } // Deep Ocean
                 else if height_val < 0.0 { -1 } // Shallow Water
                 else if height_val < 1.5 { 0 }  // Plains/Beach
                 else if height_val < 4.0 { 1 }  // Hills
                 else if height_val < 6.5 { 2 }  // Mountain
                 else { 3 };                     // Snow Peak

    let mut t_type = TileType::DeepWater;

    match height {
        -2 => t_type = TileType::DeepWater,
        -1 => {
            // Chance for ships or rocks in water
            if n_detail > 0.95 { t_type = TileType::Ship; }
            else if n_detail > 0.90 { t_type = TileType::DeepWater; } // Rocks logic later
            else { t_type = TileType::Water; }
        },
        0 => {
            // Coastline logic
            if height_val < 0.2 { t_type = TileType::Sand; }
            else if n_bio > 0.6 { t_type = TileType::Forest; }
            else if n_bio < 0.2 { t_type = TileType::SandDesert; }
            else { t_type = TileType::Grass; }
        },
        1 => {
            if n_bio > 0.7 { t_type = TileType::Forest; }
            else { t_type = TileType::Hill; }
        },
        2 => t_type = TileType::Mountain,
        _ => t_type = TileType::MountainSnow,
    }

    // 4. Feature Generation (Rivers, Roads, Buildings)
    // We override terrain with features based on noise patterns
    
    // Rivers (Sinusoidal paths)
    let river_v = (q as f32 * 0.1 + n_elev * 2.0).sin();
    if height >= 0 && height < 2 && river_v.abs() < 0.05 {
        t_type = TileType::River;
        // Flatten terrain for river
        return HexData { tile_type: t_type, height: 0 };
    }

    // Roads (Connecting distinct areas)
    let road_v = (r as f32 * 0.1 + n_bio * 2.0).cos();
    if height == 0 && road_v.abs() < 0.05 && t_type != TileType::River {
        t_type = TileType::Path;
    }

    // 5. Building Placement (Context Sensitive)
    if t_type == TileType::Grass || t_type == TileType::SandDesert {
        if n_detail > 0.96 { t_type = TileType::Castle; }
        else if n_detail > 0.94 { t_type = TileType::WizardTower; }
        else if n_detail > 0.90 { t_type = TileType::Market; }
        else if n_detail > 0.88 { t_type = TileType::Mill; }
        else if n_detail > 0.85 { t_type = TileType::Farm; }
        else if n_detail > 0.83 { t_type = TileType::Sheep; }
    }
    
    if t_type == TileType::Hill && n_detail > 0.9 {
        t_type = TileType::Mine;
    }
    
    if t_type == TileType::Forest && n_detail > 0.9 {
        t_type = TileType::Lumber;
    }

    // Coast Buildings
    if t_type == TileType::Sand && n_detail > 0.8 {
        t_type = TileType::Dock;
    }
...
fn spawn_hex(
    ...
    // --- 1. TERRAIN STACKING (CLIFfs) ---
    // Stack dirt blocks to create elevation
    ...
    for h in stack_start..=data.height {
        ...
        // Skirt/Foundation
        if h == stack_start && h != -2 {
             commands.spawn((
                SceneRoot(asset_server.load("dirt.glb#Scene0")),
                Transform::from_xyz(0.0, layer_y - (settings.height_step * 0.5), 0.0)
                    .with_scale(scale_vec * Vec3::new(0.9, 0.9, 0.9)), 
            )).set_parent(parent);
        }

        if !is_top {
            // Filler block (Stone for mountains, Dirt for others)
            let filler = if data.tile_type == TileType::Mountain || data.tile_type == TileType::MountainSnow 
                         { "stone.glb" } else { "dirt.glb" };
            commands.spawn((
                SceneRoot(asset_server.load(format!("{}#Scene0", filler))),
        ...
        let mut model = "dirt.glb".to_string();
        let mut rotation = 0.0;
        let mut overlay_model: Option<String> = None;

        match data.tile_type {
            // --- TERRAIN ---
            TileType::Water => model = "water.glb".to_string(),
            TileType::DeepWater => model = "water.glb".to_string(),
            TileType::Sand => model = "sand.glb".to_string(),
            TileType::SandDesert => model = "sand-desert.glb".to_string(),
            TileType::Grass => model = "grass.glb".to_string(),
            TileType::Forest => {
                model = "grass.glb".to_string();
                // Randomly pick forest density
                overlay_model = Some(if rand::random::<f32>() > 0.5 { "grass-forest.glb".to_string() } else { "unit-tree.glb".to_string() });
            },
            TileType::Dirt => model = "dirt.glb".to_string(),
            TileType::Hill => model = "grass-hill.glb".to_string(),
            TileType::Mountain => model = "stone-mountain.glb".to_string(),
            TileType::MountainSnow => model = "stone-mountain.glb".to_string(), // Need to color this white or use snow asset if available
            TileType::Stone => model = "stone.glb".to_string(),

            // --- CONNECTED PATHS ---
            TileType::River => {
                let mask = calculate_neighbor_mask(q, r, &grid.tile_data, TileType::River);
                let (m, r) = get_advanced_connection(mask, "river");
                model = "dirt.glb".to_string(); // River bed
                spawn_custom_model(commands, asset_server, &m, pos, layer_y - 0.2, r, settings.tile_scale);
            },
            TileType::Path => {
                let mask = calculate_neighbor_mask(q, r, &grid.tile_data, TileType::Path);
                let (m, r) = get_advanced_connection(mask, "path");
                model = "grass.glb".to_string();
                spawn_custom_model(commands, asset_server, &m, pos, layer_y + 0.05, r, settings.tile_scale);
            },
            TileType::Bridge => {
                model = "bridge.glb".to_string();
                rotation = PI / 2.0;
                spawn_custom_model(commands, asset_server, "river-straight.glb", pos, layer_y - 0.2, 0.0, settings.tile_scale);
            },

            // --- BUILDINGS (On top of Grass) ---
            TileType::Castle => { model = "grass.glb".to_string(); overlay_model = Some("building-castle.glb".to_string()); },
            TileType::Tower => { model = "grass.glb".to_string(); overlay_model = Some("building-tower.glb".to_string()); },
            TileType::WallTower => { model = "grass.glb".to_string(); overlay_model = Some("unit-wall-tower.glb".to_string()); },
            TileType::WizardTower => { model = "grass.glb".to_string(); overlay_model = Some("building-wizard-tower.glb".to_string()); },
            
            TileType::Market => { model = "grass.glb".to_string(); overlay_model = Some("building-market.glb".to_string()); },
            TileType::Mill => { model = "grass.glb".to_string(); overlay_model = Some("building-mill.glb".to_string()); },
            TileType::Watermill => { model = "grass.glb".to_string(); overlay_model = Some("building-watermill.glb".to_string()); },
            TileType::Farm => { model = "grass.glb".to_string(); overlay_model = Some("building-farm.glb".to_string()); },
            TileType::Mine => { model = "stone-hill.glb".to_string(); overlay_model = Some("building-mine.glb".to_string()); },
            TileType::Smelter => { model = "dirt.glb".to_string(); overlay_model = Some("building-smelter.glb".to_string()); },
            TileType::Lumber => { model = "grass.glb".to_string(); overlay_model = Some("dirt-lumber.glb".to_string()); },
            
            TileType::Port => { model = "grass.glb".to_string(); overlay_model = Some("building-port.glb".to_string()); },
            TileType::Dock => { model = "water.glb".to_string(); overlay_model = Some("building-dock.glb".to_string()); },
            
            TileType::Sheep => { model = "grass.glb".to_string(); overlay_model = Some("building-sheep.glb".to_string()); },
            
            // --- UNITS (Bobbing) ---
            TileType::Ship => {
                model = "water.glb".to_string();
                commands.entity(parent).insert(Bob { speed: 1.0, amount: 0.1, base_y: pos.y + layer_y, offset: 0.0 });
                overlay_model = Some("unit-ship-large.glb".to_string());
            },
        ...
        // Spawn Overlay (Building/Prop)
        if let Some(overlay) = overlay_model {
            commands.spawn((
                SceneRoot(asset_server.load(format!("{}#Scene0", overlay))),
                Transform::from_xyz(0.0, layer_y, 0.0).with_scale(scale_vec),
            )).set_parent(parent);
        }
    }
...
fn setup_game(
    ...
    // Save seed before moving grid
    let grid_seed = grid.seed;
    
    // Generate a large radius immediately
    let radius: i32 = 25; 
    for q in -radius..=radius {
        for r in -radius..=radius {
            if (q + r).abs() > radius { continue; }
            let data = generate_hex_data(q, r, grid_seed, settings.island_size);
            grid.tile_data.insert((q, r), data);
        }
    }
...
fn setup_player(commands: &mut Commands, meshes: &mut Assets<Mesh>, materials: &mut Assets<StandardMaterial>, _assets: &GameAssets, pos: Vec3) {
    ...
    commands.spawn((
        ...
        Transform::from_translation(pos + Vec3::Y * 15.0), 
        Player { 
            fire_timer: 0.0,
            jump_count: 0,
            dash_timer: 0.0,
